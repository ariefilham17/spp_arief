...
<div class="middle-content container-xxl p-0">
    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
            <div class="widget-content widget-content-area br-8">
                <div class="text-end">
                    <button onclick="addPengeluaran()" class="btn btn-primary mb-2 ms-auto _effect--ripple waves-effect waves-light w-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus">
                            <line x1="12" y1="5" x2="12" y2="19"></line>
                            <line x1="5" y1="12" x2="19" y2="12"></line>
                        </svg><span class="icon-name"></span>
                        <span class="btn-text-inner">Pengeluaran</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="modal-pengeluaran" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form onsubmit="savePengeluaran(); return false;">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#zero-config').on('click', '.delete', function() {
            ...
            deletePengeluaran(id, kode);
        });
    })

    const addPengeluaran = () => {
        $('#modal-pengeluaran .modal-title').text('Tambah Pengeluaran');
        $('#modal-pengeluaran form').trigger('reset');
        $('#modal-pengeluaran form #id').val('');
        jenis.setValue(null);
        $('#modal-pengeluaran').modal('show');
    }

    const editPengeluaran = (id, kode) => {
        $.get(base_url + 'api/pengeluaran/' + id).then((res) => {
            console.log(res);
            $('#modal-pengeluaran .modal-title').text('Edit Pengeluaran');
            $('#modal-pengeluaran form').trigger('reset');
            $('#modal-pengeluaran form #id').val(res?.id);
            $('#modal-pengeluaran form #deskripsi').val(res?.deskripsi);
            $('#modal-pengeluaran form #nominal').val(res?.debit).trigger('keyup');
    ...
    const savePengeluaran = () => {
        let id = $('#modal-pengeluaran #id').val() || false;

        let data = {
            id: $('#modal-pengeluaran #id').val(),
            jenis: $('#modal-pengeluaran #select-kategori').val(),
            deskripsi: $('#modal-pengeluaran #deskripsi').val(),
            nominal: $('#modal-pengeluaran #nominal').val().replaceAll(',', ''),
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: id ? "PUT" : "POST",
                    url: base_url + 'api/pengeluaran' + (id ? "/" + id : ""),
    ...
    const deletePengeluaran = (id, kode) => {
        let data = {
            id
        };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: base_url + 'api/pengeluaran/' + id,
                    data: JSON.stringify(data),
                    contentType: 'application/json',
                    success: function(res) {
                        Swal.fire({

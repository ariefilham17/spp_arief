    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
            <div class="widget-content widget-content-area br-8">
                <div class="text-end">
                    <button onclick="addJenisSiswa()" class="btn btn-primary mb-2 ms-auto _effect--ripple waves-effect waves-light w-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus">
                            <line x1="12" y1="5" x2="12" y2="19"></line>
                            <line x1="5" y1="12" x2="19" y2="12"></line>
                        </svg><span class="icon-name"></span>
                        <span class="btn-text-inner">Jenis Siswa</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="modal-jenis" tabindex="-1" aria-labelledby="exampleModalCenterTitle" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form onsubmit="saveJenisSiswa(); return false;">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <div class="mb-2">
                                ...
                                <input id="kode" type="text" name="kode" placeholder="Masukkan Kode" class="form-control" required="" onkeyup="formatNumber(this)">
                            </div>
                            <div class="mb-2">
...
<script>
    $(document).ready(function() {
        ...
        $('#zero-config').on('click', '.delete', function() {
            ...
            deleteJenisSiswa(id, nama);
        });
    })

    const addJenisSiswa = () => {
        $('#modal-jenis .modal-title').text('Tambah Jenis Siswa');
        $('#modal-jenis form').trigger('reset');
        $('#modal-jenis form #id').val('');
        $('#modal-jenis').modal('show');
    }

    const editJenisSiswa = (id, nama) => {
        $.get(`${base_url}/api/jenis_siswa/${id}`).then((res) => {
            $('#modal-jenis .modal-title').text('Edit Jenis Siswa');
            $('#modal-jenis form').trigger('reset');
            $('#modal-jenis form #id').val(res?.id);
            $('#modal-jenis form #kode').val(res?.kode);
            $('#modal-jenis form #nama').val(res?.nama);
            $('#modal-jenis').modal('show');
        });
    }

    const saveJenisSiswa = () => {
        let id = $('#modal-jenis #id').val() || false;

        let data = {
            id: $('#modal-jenis #id').val(),
            kode: $('#modal-jenis #kode').val(),
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: id ? "PUT" : "POST",
                    url: base_url + 'api/jenis_siswa' + (id ? "/" + id : ""),
    ...
    const deleteJenisSiswa = (id, nama) => {
        let data = {
            id
        };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: base_url + 'api/jenis_siswa/' + id,
                    data: JSON.stringify(data),
                    contentType: 'application/json',
                    success: function(res) {
                        Swal.fire({
